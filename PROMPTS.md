# Meridiax — Build Log & Prompt Registry
...
18. **Newsletter Subscription** *(Task 10)*
    ...
    - Newsletter HTML: professional dark-mode table-based email template with MRX branding, gradient header, color-coded impact badges, implication rows, footer
...
- [x] ~~Task 46 - Contact Us overlay (z-index 4600) implemented. Triggers: ✉ CONTACT button in topbar-right (hidden ≤768px) + "✉ CONTACT US" in About page footer. Form: 5 category pill-selectors (🐛 Bug Report · 💡 Feature Request · 📊 Data Request · 🤝 Partnership · 💬 General), Subject, Message, optional Reply Email. Submit builds mailto:contactmacroterminal@gmail.com pre-filled with category+subject+body. ⧉ COPY EMAIL copies address with ✓ toast. ESC closes. Click-outside closes. Form resets on open. 482/482 passing.~~
- [x] ~~Task 47 - Advanced chart upgrade for professional traders. Replaced MA5/MA20 Simple MA buttons with EMA9/EMA21/EMA50/EMA200 overlay lines (true exponential MAs, colour-coded amber/cyan/purple/pink). Added MACD(12,26,9) sub-pane (histogram + MACD/signal lines), Stochastic(14,3,3) sub-pane (%K/%D + 20/80 bands). Oscillator selector (RSI | MACD | STOCH | OFF) — radio-style, only one sub-indicator active at a time. Extended periods from 3M/6M/ALL → 1M/3M/6M/1Y/ALL (real data) or 3M/6M/ALL (simulated). AI-derived Support/Resistance levels drawn as dashed green/red horizontal price lines from payload. Analysis info overlay panel (top-left, non-blocking) shows trend arrow, S/R values, AI trade signal with direction + conviction % + entry/target/stop. Crosshair OHLC bar updated with active EMA values (E9/E21/E50/E200) and oscillator value (RSI / MACD+SIG / %K+%D). HI/LO stats enhanced with % distance from last price. Both commodity and FX expand-chart call sites updated to pass support, resistance, trend, tradeIdea.~~

- [x] ~~Task 48 - Chart data review: reliable 5-year history, dual-tier fetch, proxy fallback, adjusted close. (1) fetchYFOHLC upgraded: two-proxy fallback chain (corsproxy.io → api.allorigins.win), 10s AbortController timeout, adjusted close used for accurate long-term prices (splits/dividends corrected), null bar filtering, timestamp sort + deduplication. (2) Commodity and FX background-fetch now dual-tier: 5Y weekly (`5y 1wk`, ~260 bars) stored as ohlcvWeekly for ALL/5Y view; 1Y daily (`1y 1d`, ~252 bars) stored as ohlcvDaily for 1M/3M/6M/1Y views. (3) drawChartFsLine period routing: daily data selected for 1M/3M/6M/1Y (better granularity), weekly for ALL (5-year history). (4) ALL period button relabelled "5Y" when 5Y weekly data is available. (5) Country macro history chart extended from 12 quarters (Q1 2022–Q4 2024) to 20 quarters (Q1 2020–Q4 2024) covering COVID crash, recovery, inflation surge, rate hike cycle — full 5-year economic cycle. 95/96 tests passing (1 stale test ID, not a real bug).~~

- [x] ~~Task 49 - Trader UX review & 7 new features. Research-backed audit of professional macro terminal UX patterns. Implemented: (1) Mini-ticker bar — fixed bottom bar (26px) showing live DXY/GOLD/WTI/VIX/US10Y with colour-coded % change, UTC clock, and CALENDAR/SEARCH quick-access buttons; `applyMiniTicker()` wired into existing `applyTickerData()` + `loadTickers()` pipeline. (2) Economic Calendar overlay — AI-generated 4-week forward event list (FOMC, ECB, NFP, CPI, PMIs, etc.) with category/importance filters; 12-hour cache TTL; grouped by week with importance dot indicators. (3) Ctrl+K / `/` Quick Search — full-screen overlay with lazy-built 111-item index (195 countries + 16 FX pairs + 11 commodities + 7 nav actions); arrow-key navigation; `_buildSearchIndex()` called once on first open. (4) Keyboard Shortcuts Panel — `?` key toggles bottom-right cheat-sheet overlay; auto-closes on click-outside. (5) Global keyboard shortcuts — single `keydown` listener: F=FX, X=Commodities, R=Research, E=Calendar, B=Community, P=Profile, A=Custom Analysis, Ctrl+K/`/`=Search, `?`=Shortcuts, ESC=close priority stack. (6) Trend arrows on panel chips — ▲/▼/◆ appended to rate_outlook (Hiking/Cutting/Stable) and fx_bias (Strengthen/Weaken) chips in `renderPanel`. (7) Dynamic page title — `selectCountry` sets `document.title = "${country} — Meridiax"`. 102/102 tests passing.~~

- [x] ~~Task 50 - Pre-deployment review: 7 bugs fixed. Audit of all overlays, chart logic, mobile responsiveness. Bugs fixed: (1) `chartFsPeriod` not reset when opening history chart — GDP data was being incorrectly sliced to 3/6 quarters if user had previously selected that period on a commodity chart; fixed by resetting to `'ALL'` in `openChartFs` when `type === 'history'`. (2) EMA9/21/50/200 + BB + RSI/MACD/STOCH toolbar buttons shown for macro history chart (GDP/CPI data) — meaningless for quarterly macro series; moved all indicator buttons inside the `else` block (price charts only). (3) US10Y change element showed stray `%` before data loaded — removed from HTML placeholder. (4) Economic calendar cache never expired — added `calCacheTs` timestamp + 12-hour `CAL_TTL`; stale check on `openCalendar()`. (5) Mini-ticker overflowed on mobile — added `overflow-x: auto`, hidden scrollbar, clock hidden at ≤480px. (6) Quick-search overlay had excessive padding-top on small screens — reduced 80px→60px at 768px, 40px at 480px. (7) Shortcuts panel visible and overlapping on mobile — hidden entirely (`display:none !important`) at ≤480px. 33/33 new tests + all prior tests passing.~~
